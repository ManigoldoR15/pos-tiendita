...
import { registrarAjustePiezaAction, type MotivoAjuste } from './actions'
...
export default function AjusteFormPieza({ productoId, productoNombre, existenciasActuales }: Props) {
  ...
  const cantidadNum = parseInt(cantidadFisica, 10)
  const delta = cantidadFisica ? cantidadNum - existenciasActuales : null
  ...
  async function handleSubmit() {
    ...
    setError(null)
    setPending(true)
    const result = await registrarAjustePiezaAction({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg bg-muted/50 px-4 py-3">
        ...
        <p className="text-2xl font-black text-primary">{existenciasActuales} pzas.</p>
      </div>

      {/* Cantidad física contada */}
      <div className="space-y-1.5">
        <label className="text-sm font-medium">¿Cuántas contaste físicamente? (piezas)</label>
        <input
          type="number"
          min="0"
          step="1"
          ...
          className="w-full rounded-lg border border-input bg-background px-3 py-3 text-xl font-bold outline-none focus:ring-2 focus:ring-ring"
        />
      </div>
