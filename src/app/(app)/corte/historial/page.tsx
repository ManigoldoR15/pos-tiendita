...
export default async function HistorialCortesPage({
  ...
  if (rol === 'empleado') redirect('/corte')
  ...
  const [{ data: turnos }, { data: miembros }] = await Promise.all([
  ...
  const cajeros = ((miembros ?? []) as MiembroRow[]).filter((m) => m.rol !== 'dueno')
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight">Historial de cortes de caja</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Turnos de caja cerrados · solo visible para el dueño
            </p>
          </div>
        </div>
      </div>
      ...
      <form method="get" className="flex flex-wrap gap-3 items-end">
        ...
        {(cajeroId || desde || hasta) && (
          <Link
            href={`/corte/historial?vista=${vista}`}
        ...
        <div className="ml-auto flex rounded-lg border overflow-hidden text-sm">
          <Link
            href={`/corte/historial?${new URLSearchParams({ ...(cajeroId ? { cajero: cajeroId } : {}), ...(desde ? { desde } : {}), ...(hasta ? { hasta } : {}), vista: 'lista' }).toString()}`}
            className={[
          ...
          <Link
            href={`/corte/historial?${new URLSearchParams({ ...(cajeroId ? { cajero: cajeroId } : {}), ...(desde ? { desde } : {}), ...(hasta ? { hasta } : {}), vista: 'calendario' }).toString()}`}
            className={[
              'px-3 py-2 font-medium transition-colors',
        ...
        <div className="card-soft p-5 max-w-sm">
          <CalendarioTurnos
            turnos={lista.map((t) => ({
              ...
              apertura_email: t.apertura_email ?? '',
                ...
                return (
                  <tr key={t.id} className="hover:bg-muted/20 transition-colors">
                    <td className="px-4 py-3 whitespace-nowrap">
                      {fmtFechaHoraCorta(t.fecha_cierre)}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {t.cerrado_por ? (
                        <>
                          <span className="font-medium">{emailCajero(t.cierre_email)}</span>
                          {t.cerrado_por !== t.abierto_por && (
