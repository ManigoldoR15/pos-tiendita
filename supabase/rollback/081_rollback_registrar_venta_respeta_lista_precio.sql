-- Rollback de 081: registrar_venta vuelve a ignorar la lista de precios y a
-- cobrar siempre productos.precio_venta. Quita la firma de 9 parámetros y
-- restaura la de 8.

DROP FUNCTION IF EXISTS public.registrar_venta(uuid, jsonb, uuid, uuid, integer, integer, uuid, uuid, uuid);
...
REVOKE EXECUTE ON FUNCTION public.registrar_venta(uuid, jsonb, uuid, uuid, integer, integer, uuid, uuid) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.registrar_venta(uuid, jsonb, uuid, uuid, integer, integer, uuid, uuid) TO authenticated, service_role;
