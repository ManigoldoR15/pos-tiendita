-- =============================================================================
-- MIGRACIÓN 041: sa_lista_negocios unificada (Fase 1 — panel superadmin único)
--
-- Extiende sa_lista_negocios con los campos que hasta ahora solo entregaba
-- get_todos_negocios (panel /admin-sistema): teléfono, ubicación, fechas de
-- suscripción y estado_suscripcion calculado. Con esto la lista única de
-- /superadmin/negocios puede absorber al panel admin-sistema sin perder datos.
--
-- Solo toca RPCs consumidos por el panel de superadmin. No modifica tablas
-- ni nada que use la app de los negocios. get_todos_negocios queda intacta
-- (se retirará cuando muera el panel viejo).
--
-- Rollback: supabase/rollback/041_rollback_sa_lista_unificada.sql
-- =============================================================================

-- El tipo de retorno cambia → hay que dropear primero
...
COMMENT ON FUNCTION sa_lista_negocios IS
  'Lista unificada de negocios para el panel superadmin: operación (ventas, '
  'actividad, catálogo, plazas) + cuenta (plan, suscripción, estado_suscripcion, '
  'contacto del dueño). Solo superadmins.';
