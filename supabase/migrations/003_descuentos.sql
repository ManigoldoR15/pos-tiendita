-- Agregar columna descuento a ventas (centavos, 0 = sin descuento)
ALTER TABLE ventas ADD COLUMN IF NOT EXISTS descuento integer NOT NULL DEFAULT 0;

-- Eliminar firma anterior (5 params) para evitar ambigüedad en PostgREST rpc()
DROP FUNCTION IF EXISTS registrar_venta(uuid, jsonb, uuid, uuid, integer);

-- Reemplazar registrar_venta con soporte de descuento opcional
...
BEGIN
  ...
  -- Aplicar descuento (nunca puede quedar negativo)
  v_total := GREATEST(0, v_total - p_descuento);

  -- Validar pago recibido contra total ya descontado
  ...
  -- Insertar cabecera de venta con descuento
  INSERT INTO ventas (
...
COMMENT ON FUNCTION registrar_venta(uuid, jsonb, uuid, uuid, integer, integer) IS
  'Registra una venta con descuento opcional (p_descuento en centavos). '
  'Valida stock, aplica descuento al total, inserta venta + items con precio snapshot '
  'y descuenta existencias. Items: [{"producto_id":"uuid","cantidad":2}].';
